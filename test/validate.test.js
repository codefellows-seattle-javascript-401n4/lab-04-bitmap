...
describe('Validate', function(){
  ...
  it('should fail if only "node index.js" is entered in', function(done) {
    let args = baseArgs;
  ...
  it('should not fail if correct inputs are used', function(done) {
    let args = baseArgs.concat('bitmap.bmp','newbitmap.bmp', 'invert');
    expect(validate(args, opts)).toEqual(true);
    done();
  });

  it('should fail if a non-bitmap input file is used', function(done) {
    let args = baseArgs.concat('bitmap.jpg','newbitmap.bmp', 'invert');
    expect(validate(args, opts)).toEqual(false);
    done();
  });

  it('should fail if a non-bitmap output file is used', function(done) {
