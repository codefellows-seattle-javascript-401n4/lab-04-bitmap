...
BitMap.prototype.palletUpdate = function(){
  ...
  Object.keys(this.palletColors)
    .map((key, index) => {
      this.pallet[key] = this.palletColors[key].red
      this.pallet[Number(key) + 1] = this.palletColors[key].green
      this.pallet[Number(key) + 2] = this.palletColors[key].blue
      this.pallet[Number(key) + 3] = this.palletColors[key].alpha
  })
  console.log(this.palletColors);
...
BitMap.prototype.write = function(){
  ...
  writeFile(__dirname + '/../asset/testing.bmp', this.buffer)
}
